...
Migration.confirmWriteToDisk = function(_, _, stop) { stop(null); };
Migration.confirmUpdateInDb  = function(_, _, stop) { stop(null); };
        ...
        async.auto({
          ...
          hasUpdatedMigration: ['migration', function(cb, d) {
            ...
            var stop = function(err) {
              if (err) { return cb(err); }
              return cb(new Error('Declined to update a modifed migration in the database'));
            };

            (opts.confirmUpdateInDb || _this.confirmUpdateInDb)(d.migration, next, stop);
          }],
