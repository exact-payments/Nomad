...
class Nomad {
  ...
  up(target, opts = {}, cb = noop) {
    ...
    const canReverseDevergentMigrations = opts.canReverseDevergentMigrations || (() => true);
    const canApplyUnreversibleMigration = opts.canApplyUnreversibleMigration || (() => true);
    ...
    this.getMigrations((err, migrations) => {
      ...
      const applyPendingMigrations = (cb) => {
        ...
        for (const migration of pendingMigrations) {
          if (!migration.isReversible && !canApplyUnreversibleMigration(migration)) {
            return cb(new MigrationError(
      ...
      const reverseDivergentMigrations = (cb) => {
        async.each(migrations.devergent.reverse(), (migration, cb) => {
      ...
      if (migrations.devergent.some(m => !m.isReversible)) {
        return cb(new MigrationError(
          'unreversibleDevergentMigration',
          'Cannot perform up command. A devergent migration cannot reversed. Manual intervention ' +
          'required.'
        ));
      }

      if (migrations.devergent.length > 0) {
        if (!canReverseDevergentMigrations(migrations.devergent)) {
          return cb(new MigrationError(
            'abortReverseDevergentMigration',
            'Cannot perform up command. Reversal of devergent migrations aborted.'
          ));
        }
        return reverseDivergentMigrations(cb);
      }
  ...
  down(target, opts = {}, cb = noop) {
    ...
    this.getMigrations((err, migrations) => {
      ...
      let appliedMigrations = [].concat(migrations.applied, migrations.divergent).reverse();
      if (target) {
        ...
        appliedMigrations = appliedMigrations.slice(0, targetIndex);
      }
  ...
  getMigrations(cb = noop) {
    this.database.getMigrations((err, migrations) => {
      ...
      const devergent = migrations.slice(headIndex).filter(m => !!m.appliedSrc);
      const applied   = migrations.slice(0, headIndex);

      cb(null, { unapplied, devergent, applied });
    });
  }

  syncDatabaseAndDisk(opts = {}, cb = noop) {
    if (!this.nomadFile) {
      return cb(new MigrationError(
        'nomadFileNotLoaded',
